...
namespace Engine {
    ...
    public:
        ...
        // built; the Entities list, Inspector, IBL window, and
        // Post-Processing window only in AppMode::Debug — in AppMode::Game
        // the mouse is captured by the camera, so they couldn't be clicked
        // anyway. Pure CPU-side widget construction, no GPU calls — can run
        // at any point before FinalizeDrawData. iblSettings/
        // postProcessSettings are edited in place by their windows'
        // sliders, the same pattern DrawInspector already uses for
        // component fields.
    ...
    private:
        ...
        void DrawPostProcessSettings(PostProcessSettings &postProcessSettings);
